...
export function AlumnoPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const [p, list, professors, mentors] = await Promise.all([
      ...
      setMentorName(mentors.find((x) => x.id === p.mentorId)?.name ?? p.responsableNombre)
  ...
  if (!loading && profile && !profile.setupCompleted) {
    return <Navigate to="/alumno/onboarding" replace />
  }
              ...
              {WEEKDAYS.map(({ key, label }) => (
                <div key={key} className="schedule-row">
                  ...
                  <div className="summary-item" style={{ padding: '0.75rem' }}>
                    <span>Entrada</span>
                    <strong>{profile.horariosSemana?.[key]?.entrada || '—'}</strong>
                  </div>
                  <div className="summary-item" style={{ padding: '0.75rem' }}>
                    <span>Salida</span>
                    <strong>{profile.horariosSemana?.[key]?.salida || '—'}</strong>
                  </div>
                </div>
              ))}
            </div>
