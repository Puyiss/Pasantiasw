...
let internshipPeriod: InternshipPeriodConfig = { ...DEFAULT_INTERNSHIP_PERIOD }
...
function isDateScheduleComplete(s: DateSchedule, config = internshipPeriod) {
  const dates = datesInRange(config.startDate, config.endDate)
  const filled = dates
    .map((iso) => s[iso])
    .filter((d) => d?.entrada && d?.salida)
  return filled.length >= 1 && filled.every((d) => d.entrada !== d.salida)
}
...
export async function mockRequest(
  ...
  switch (action) {
    ...
    case 'getInternshipPeriod':
    case 'getInternshipWeek':
      return internshipPeriod

    case 'updateInternshipPeriod':
    case 'updateInternshipWeek': {
      ...
      if (startDate > endDate) {
        throw new Error('La fecha de inicio no puede ser después de la de fin')
      }
      const days = datesInRange(startDate, endDate)
      if (!days.length) throw new Error('El rango de fechas está vacío')
      if (days.length > 45) {
        throw new Error('El rango no puede superar los 45 días')
      }
      internshipPeriod = { startDate, endDate }
      return internshipPeriod
    }
    ...
    case 'createCourse': {
      ...
      if (!course.name.trim()) throw new Error('El nombre del curso es obligatorio')
      if (!course.professorId) throw new Error('Debés asignar un profesor al curso')
      if (!users.some((u) => u.id === course.professorId && u.role === 'profesor')) {
    ...
    case 'deleteCourse': {
      const id = String(payload.id)
      if (!courses.some((c) => c.id === id)) throw new Error('Curso no encontrado')
      courses = courses.filter((c) => c.id !== id)
      profiles = profiles.map((p) => (p.courseId === id ? { ...p, courseId: '' } : p))
      return { deleted: id }
    }

    case 'importStudents': {
      const rows = (payload.students as Array<Record<string, string>>) ?? []
      ...
      for (const row of rows) {
        try {
          ...
          const email = String(row.email ?? '')
            .toLowerCase()
            .trim()
          const password = String(row.password ?? 'alu123') || 'alu123'
          const professorEmail = String(row.professorEmail ?? '')
            .toLowerCase()
            .trim()
          const courseName = String(row.courseName ?? '').trim()
          if (!nombre || !apellido) {
            skipped++
            errors.push('Fila sin nombre o apellido')
            continue
          }
          if (!email) {
            skipped++
            errors.push(`${nombre} ${apellido}: sin email`)
            continue
          }
          ...
          const professor = users.find(
            (u) => u.role === 'profesor' && u.email === professorEmail,
          )
          if (!professor) {
            skipped++
            errors.push(`${email}: profesor ${professorEmail || '(vacío)'} no encontrado`)
            continue
          }
          if (!courseName) {
            skipped++
            errors.push(`${email}: sin curso`)
            continue
          }
          let course = courses.find(
            (c) => c.professorId === professor.id && c.name === courseName,
          )
          if (!course) {
            course = { id: uid('c'), name: courseName, professorId: professor.id }
            courses.push(course)
          }
          const user: User = {
            id: uid('u'),
          ...
          skipped++
          errors.push(err instanceof Error ? err.message : 'Error en fila')
        }
      }
      return { created, skipped, errors: errors.slice(0, 12) }
    }
    ...
    case 'updateProfile': {
      ...
      let horariosFechas = current.horariosFechas ?? {}
      if (payload.horariosFechas) {
        horariosFechas = payload.horariosFechas as DateSchedule
        if (!isDateScheduleComplete(horariosFechas)) {
          throw new Error(
            'Completá al menos un día con entrada y salida dentro del período de pasantía.',
          )
        }
      }

      const activeDates = datesInRange(internshipPeriod.startDate, internshipPeriod.endDate)
      const horariosText =
        formatDateSchedule(horariosFechas, activeDates) ||
        formatWeekSchedule(horariosSemana) ||
        current.horarios
